...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        console.log(data);
        document.getElementById('users-list').innerHTML = ` ${data.active_users}`;
    } else if (data.type === "disconnect") {
        console.log('A user has just disconnected:');
        console.log(data);
        document.getElementById('users-list').querySelector('ul').innerHTML = ` ${data.active_users}`;
    } else if (data.type === "chat") {
        console.log('A user has just sent a chat message:');
        console.log(data);
        console.log(username);
        if (data.username == username){
            document.getElementById('chat').innerHTML += `<div class="right" aria-live="polite" role="textbox"> <b>you:</b> ${data.text} </div>`;
        }
        else {
            document.getElementById('chat').innerHTML += `<div class="left" aria-live="polite" role="textbox"> <b>${data.username}:</b> ${data.text} </div>`;
        }
